...
class AthleticsDataScraper:
    def __init__(self, gender):
        print(f"Initializing scraper for {gender}")
        self.base_url = 'https://www.alltime-athletics.com/'
        self.gender = gender

    def generate_url(self, event, is_legal):
        print(f"Generating URL for event: {event}, is_legal: {is_legal}")
        special_cases_male = {
    ...
    def add_age_at_time_of_race(self, df):
        ...
        df['DOB'] = df['DOB'].apply(lambda x: x if pd.isnull(x) or x.year < 2023 
                                    else pd.Timestamp(year=x.year - 100, month=x.month, day=x.day))
